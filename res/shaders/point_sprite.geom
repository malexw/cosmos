...
void main() {
  ...
  vec4 offset_tl = gl_ProjectionMatrix * vec4(-0.5, 0.5, 0.0, 1.0);

  gl_Position = gl_PositionIn[0] + offset_tl;
  tex_coords = vec2(0.0, 0.0);
  EmitVertex();

  vec4 offset_bl = gl_ProjectionMatrix * vec4(-0.5, -0.5, 0.0, 1.0);

  gl_Position = gl_PositionIn[0] + offset_bl;
  tex_coords = vec2(0.0, 1.0);
  EmitVertex();
  
  gl_Position = gl_PositionIn[0] + (gl_ProjectionMatrix * vec4(0.5, 0.5, 0.0, 1.0));
